...
function toggleLamp(id) {
    ...
    // === UPDATE FIREBASE ===
    setDB(refDB(db, "kamar_ovan/lamp" + id), newState ? 1 : 0);
}
...
function toggleFan() {
    ...
    // === UPDATE FIREBASE ===
    setDB(refDB(db, "kamar_ovan/fan"), newState ? 1 : 0);
}
...
onValue(refDB(db, "kamar_ovan"), (snapshot) => {
    const data = snapshot.val();
    if (!data) return;

    // Sync LAMPU
    if (data.lamp1 !== undefined) syncLamp(1, data.lamp1);
    if (data.lamp3 !== undefined) syncLamp(3, data.lamp3);
    if (data.lamp4 !== undefined) syncLamp(4, data.lamp4);

    // Sync FAN
    if (data.fan !== undefined) syncFan(data.fan);
});
